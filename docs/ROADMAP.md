...
- [x] automated system Back semantics — Espresso Back dismisses calendar search/day editor while preserving the visible month, and root Back is allowed to finish `MainActivity`; verified on API 36 and Android 17;
...
Phase 6, P1 calendar search/filter и automated system Back semantics реализованы до optional sync. Текущий release/runtime marker не привязан к вручную записанному commit SHA: ревизия считается runtime-проверенной только после полного зелёного обязательного Android CI на её фактическом head commit.
